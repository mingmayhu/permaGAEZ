...
def run(mask):
    ...
    elev_masked = elevation[mask & np.isfinite(elevation)]
    print('Elevation diagnostic:')
    print(f'  Min={elev_masked.min():.2f}  Max={elev_masked.max():.2f}  '
          f'Mean={elev_masked.mean():.2f}  Median={np.median(elev_masked):.2f}')
    for p in [75, 90, 95, 99]:
        print(f'  P{p}: {np.percentile(elev_masked, p):.2f}')
